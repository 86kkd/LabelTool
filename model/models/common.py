...
from utils.general import non_max_suppression, make_divisible, \
scale_coords, increment_path, xyxy2xywh, \
save_one_box, polygon_non_max_suppression
...
class Conv(nn.Module):
    ...
    def forward(self, x):
        """mark"""
        if x.shape[-1] == 3:
            return self.act(self.bn(self.conv(x.transpose(2,3).transpose(1,2))))
        else:
            return self.act(self.bn(self.conv(x)))
        
    def fuseforward(self, x):
        if x.shape[-1] == 3:
            return self.act(self.conv(x.transpose(2,3).transpose(1,2)))
        else:
