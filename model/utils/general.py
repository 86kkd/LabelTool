...
def labels_to_class_weights(labels, nc=80):
    ...
    labels = np.concatenate(labels, 0)  # labels.shape = (866643, 5) for COCO, labels.shape = (866643, 9) for polygon COCO
    classes = labels[:, 0].astype(int) # labels = [class xywh], labels = [class, xyxyxyxy] for polygon
...
def xy_4_2_xy_2_cpu(x):
    
    y=np.copy(torch.Tensor.cpu(x))
    y=y.astype(np.int32)
    z=[]
    for i in range(y.shape[0]):
        points=np.array([[y[i][0:2]],[y[i][2:4]],[y[i][4:6]],[y[i][6:8]]],dtype=np.int32)
        result=cv2.boundingRect(points)
        z.append([result[0],result[1],result[0]+result[2],result[1]+result[3]])
...
# polygon does not support a second stage classifier
...
def polygon_box_iou(boxes1, boxes2, GIoU=False, DIoU=False, CIoU=False, eps=1e-7, device="cpu", ordered=True):
    """
...
def polygon_non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        box = xy_4_2_xy_2_cpu(x[:, :8])
        box_8 = x[:, :8].clone()
        # Detections matrix nx6 (xyxy, conf, cls)
        if multi_label:
            i, j = (x[:, 9:] > conf_thres).nonzero(as_tuple=False).T
            x_c = torch.cat((box[i], x[i, j + 9, None], j[:, None].float()), 1)
            x = torch.cat((box_8[i], x[i, j + 9, None], j[:, None].float()), 1)

        else:  # best class only
            conf, j = x[:, 9:].max(1, keepdim=True)
            x_c = torch.cat((box, conf, j.float()), 1)[conf.view(-1) > conf_thres]
            x = torch.cat((box_8, conf, j.float()), 1)[conf.view(-1) > conf_thres]

        # Filter by class
        if classes is not None:
            x_c = x_c[(x_c[:, 5:6] == torch.tensor(classes, device=x.device)).any(1)]
        ...
        # Check shape
        n = x_c.shape[0]  # number of boxes
        if not n:  # no boxes
            continue
        elif n > max_nms:  # excess boxes
            x_c = x_c[x_c[:, 4].argsort(descending=True)[:max_nms]]  # sort by confidence

        # Batched NMS
        c = x_c[:, 5:6] * (0 if agnostic else max_wh)  # classes
        boxes, scores = x_c[:, :4] + c, x_c[:, 4]  # boxes (offset by class), scores
        i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS
        if i.shape[0] > max_det:  # limit detections
            i = i[:max_det]
        if merge and (1 < n < 3E3):  # Merge NMS (boxes merged using weighted mean)
            ...
            iou = box_iou(boxes[i], boxes) > iou_thres  # iou matrix
            weights = iou * scores[None]  # box weights
            x_c[i, :4] = torch.mm(weights, x[:, :4]).float() / weights.sum(1, keepdim=True)  # merged boxes
            if redundant:
                i = i[iou.sum(1) > 1]  # require redundancy
...
def polygon_non_max_suppression_0(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=300):
    """
        Runs Non-Maximum Suppression (NMS) on inference results for polygon boxes
        Returns:  list of detections, on (n,10) tensor per image [xyxyxyxy, conf, cls]
